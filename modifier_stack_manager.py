...
bpy.utils.unregister_class(properties_data_modifier.DATA_PT_modifiers)
    ...
    def execute(self, context):
        ...
        if len(obj.modifiers) == 0:
            ...
            try:
                ...
                self.report({'INFO'}, 'Failed to apply modifier.')
        return {'FINISHED'}

class MyModifierButtonsPanel:
...
def register():
    ...
    for cl in cls:
        register_class(cl)
    bpy.types.DATA_PT_my_modifiers.prepend(draw)        

def unregister():
    from bpy.utils import unregister_class
    for cl in cls:
        unregister_class(cl)
    bpy.types.DATA_PT_my_modifiers.remove(draw)
